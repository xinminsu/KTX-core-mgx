...
describe("TokenManager", function () {
  ...
  it("signalApprove", async () => {
    await expect(
      tokenManager
        .connect(user0)
        .signalApprove(eth.address, user2.address, expandDecimals(5, 18))
  ...
  it("signApprove", async () => {
    await expect(
      tokenManager
        .connect(user0)
        .signApprove(eth.address, user2.address, expandDecimals(5, 18), 1)
    ).to.be.revertedWith("TokenManager: forbidden");
  ...
  it("approve", async () => {
    ...
    await expect(
      tokenManager
        .connect(user0)
        .approve(eth.address, user2.address, expandDecimals(5, 18), 1)
    ).to.be.revertedWith("TokenManager: forbidden");
    ...
    await expect(
      tokenManager
        .connect(wallet)
        .approve(gmx.address, user2.address, expandDecimals(5, 18), 1)
    ).to.be.revertedWith("TokenManager: action not signalled");

    await expect(
      tokenManager
        .connect(wallet)
        .approve(eth.address, user0.address, expandDecimals(5, 18), 1)
    ).to.be.revertedWith("TokenManager: action not signalled");

    await expect(
      tokenManager
        .connect(wallet)
        .approve(eth.address, user2.address, expandDecimals(6, 18), 1)
    ).to.be.revertedWith("TokenManager: action not signalled");
    ...
    await eth
      .connect(user2)
      .transferFrom(tokenManager.address, user1.address, expandDecimals(5, 18));
  ...
  it("signalSetGov", async () => {
    await expect(
      tokenManager
        .connect(user0)
        .signalSetGov(timelock.address, gmx.address, user1.address)
    ).to.be.revertedWith("TokenManager: forbidden");
  ...
  it("signSetGov", async () => {
    await expect(
      tokenManager
        .connect(user0)
        .signSetGov(timelock.address, gmx.address, user1.address, 1)
    ).to.be.revertedWith("TokenManager: forbidden");
  ...
  it("setGov", async () => {
    ...
    await expect(
      tokenManager
        .connect(user0)
        .setGov(gmxTimelock.address, gmx.address, user1.address, 1)
    ).to.be.revertedWith("TokenManager: forbidden");
    ...
    await tokenManager
      .connect(wallet)
      .signalSetGov(gmxTimelock.address, gmx.address, user1.address);

    await expect(
      tokenManager
        .connect(wallet)
        .setGov(user2.address, gmx.address, user1.address, 1)
    ).to.be.revertedWith("TokenManager: action not signalled");

    await expect(
      tokenManager
        .connect(wallet)
        .setGov(gmxTimelock.address, user0.address, user1.address, 1)
    ).to.be.revertedWith("TokenManager: action not signalled");

    await expect(
      tokenManager
        .connect(wallet)
        .setGov(gmxTimelock.address, gmx.address, user2.address, 1)
    ).to.be.revertedWith("TokenManager: action not signalled");

    await expect(
      tokenManager
        .connect(wallet)
        .setGov(gmxTimelock.address, gmx.address, user1.address, 1 + 1)
    ).to.be.revertedWith("TokenManager: action not signalled");
    ...
    await tokenManager
      .connect(signer0)
      .signSetGov(gmxTimelock.address, gmx.address, user1.address, 1);
    ...
    await expect(
      gmxTimelock.connect(wallet).signalSetGov(gmx.address, user1.address)
    ...
    await expect(
      gmxTimelock.connect(wallet).setGov(gmx.address, user1.address)
