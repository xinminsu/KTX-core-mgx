...
describe("Vault.decreaseLongPosition", function () {
  ...
  beforeEach(async () => {
    ...
    const { vaultUtils: _vaultUtils } = await initVault(
      vault,
      router,
      usdg,
      vaultPriceFeed
  ...
  it("decreasePosition long with different fee", async () => {
    await vaultUtils.setDynamicFeePositionRate(
      10,
      10,
      10,
    ...
    expect(await vault.feeReserves(btc.address)).eq(969);
    expect(await vault.reservedAmounts(btc.address)).eq(225000);
    expect(await vault.poolAmounts(btc.address)).eq(274031);
    expect(await vault.guaranteedUsd(btc.address)).eq(toUsd(80.09));
    expect(await vault.poolAmounts(btc.address)).eq(274250 - 219);
    expect(await btc.balanceOf(user2.address)).eq(0);
    ...
    // decrease position will decrease poool before calculating margin fee
    // pool utilization: 100000/274031 = 36%
    const tx = await vault
      .connect(user0)
      .decreasePosition(
        user0.address,
    ...
    expect(position[0]).eq(toUsd(40)); // size
    expect(position[1]).eq(toUsd(9.38)); // collateral
    expect(position[2]).eq(toNormalizedPrice(41000)); // averagePrice
    expect(position[3]).eq(0); // entryFundingRate
    expect(position[4]).eq(100000); // reserveAmount, 0.00100 * 40,000 => 40
    expect(position[5]).eq(toUsd(0.5)); // pnl
    ...
    expect(await vault.feeReserves(btc.address)).eq(969 + 73); // 0.00000073 * 40790 => ~0.03 USD
    expect(await vault.reservedAmounts(btc.address)).eq(100000);
    expect(await vault.poolAmounts(btc.address)).eq(273958);
    expect(await vault.guaranteedUsd(btc.address)).eq(toUsd(30.62));
    expect(await vault.poolAmounts(btc.address)).eq(274250 - 219 - 73);
    expect(await btc.balanceOf(user2.address)).eq(0);
    ...
    expect(await vault.feeReserves(btc.address)).eq(969 + 73 + 98); // 0.00000098 * 40790 => ~0.04 USD
    expect(await vault.reservedAmounts(btc.address)).eq(0);
    expect(await vault.guaranteedUsd(btc.address)).eq(0);
    expect(await vault.poolAmounts(btc.address)).eq(
      274250 - 219 - 122 - 98 - 21868
    );
    expect(await btc.balanceOf(user2.address)).eq(21917); // 0.00021917 * 40790 => ~8.94 USD
