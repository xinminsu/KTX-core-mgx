...
describe("Vault.buyUSDG", function () {
  ...
  it("buyUSDG", async () => {
    await expect(vault.buyUSDG(bnb.address, wallet.address)).to.be.revertedWith(
      "Vault: _token not whitelisted"
    );

    await expect(
      vault.connect(user0).buyUSDG(bnb.address, user1.address)
    ...
    const tx = await vault
      .connect(user0)
      .buyUSDG(bnb.address, user1.address, { gasPrice: "10000000000" });
  ...
  it("buyUSDG allows gov to mint", async () => {
    ...
    await expect(
      vault.connect(user0).buyUSDG(bnb.address, wallet.address)
  ...
  it("buyUSDG adjusts for decimals", async () => {
    ...
    await expect(
      vault.connect(user0).buyUSDG(btc.address, user1.address)
    ).to.be.revertedWith("Vault: invalid tokenAmount");
