...
describe("Vault.averagePrice", function () {
  ...
  beforeEach(async () => {
    ...
    initVaultResult = await initVault(vault, router, usdg, vaultPriceFeed);
    await initVaultResult.vaultUtils.setUseDynamicPositionFee(true);
    await initVaultResult.vaultUtils.setDynamicFeePositionRate(
      10,
      10,
      10,
      0,
  ...
  it("long position.averagePrice, dynamic fee", async () => {
    await initVaultResult.vaultUtils.setUseDynamicPositionFee(true);
    await initVaultResult.vaultUtils.setDynamicFeePositionRate(
      0,
      5,
    ...
    expect(position[0]).eq(toUsd(90)); // size
    expect(position[1]).eq(toUsd(10)); // collateral, 10 - zero fee
    expect(position[2]).eq(toNormalizedPrice(40000)); // averagePrice
    expect(position[3]).eq(0); // entryFundingRate
    expect(position[4]).eq(225000); // reserveAmount, 0.00225 * 40,000 => 90
    expect(await vault.reservedAmounts(btc.address)).eq(225000);
    ...
    expect(position[0]).eq(toUsd(100)); // size
    expect(position[1]).eq(toUsd(10 + 9.992)); // collateral
    expect(position[2]).eq(toNormalizedPrice(40000)); // averagePrice
    expect(position[3]).eq(0); // entryFundingRate
    expect(position[4]).eq(225000 + 25000); // reserveAmount
  ...
  it("long position.averagePrice, dynamic fee 2", async () => {
    await initVaultResult.vaultUtils.setUseDynamicPositionFee(true);
    await initVaultResult.vaultUtils.setDynamicFeePositionRate(
      6,
      6,
    ...
    expect(position[0]).eq(toUsd(90)); // size
    expect(position[1]).eq(toUsd(10 - 90 * 0.0006)); // 10 * (1-0.01%), cuz 0 pool utilization
    expect(position[2]).eq(toNormalizedPrice(40000)); // averagePrice
    expect(position[3]).eq(0); // entryFundingRate
    expect(position[4]).eq(225000); // reserveAmount, 0.00225 * 40,000 => 90
    expect(await vault.reservedAmounts(btc.address)).eq(225000);
    expect(await vault.poolAmounts(btc.address)).eq(274115);
    ...
    expect(position[0]).eq(toUsd(100)); // size
    expect(position[1]).eq(toUsd(20 - 90 * 0.0006 - 10 * 0.0009)); // utilization is 80%, so rate is 0.0009
    expect(position[2]).eq(toNormalizedPrice(40000)); // averagePrice
    expect(position[3]).eq(0); // entryFundingRate
    expect(position[4]).eq(225000 + 25000); // reserveAmount
  ...
  it("long position.averagePrice, dynamic fee 3", async () => {
    await initVaultResult.vaultUtils.setUseDynamicPositionFee(true);
    await initVaultResult.vaultUtils.setDynamicFeePositionRate(
      6,
      12,
  ...
  it("short position.averagePrice, dynamic fee", async () => {
    await initVaultResult.vaultUtils.setUseDynamicPositionFee(true);
    await initVaultResult.vaultUtils.setDynamicFeePositionRate(
      10,
      5,
    ...
    expect(position[0]).eq(toUsd(100)); // size
    expect(position[1]).eq("49908000000000000000000000000000"); // collateral
    expect(position[2]).eq(toNormalizedPrice(40000)); // averagePrice
    expect(position[3]).eq(0); // entryFundingRate
    expect(position[4]).eq(expandDecimals(100, 18)); // reserveAmount
