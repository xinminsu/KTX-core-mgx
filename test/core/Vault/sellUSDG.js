...
describe("Vault.sellUSDG", function () {
  ...
  it("sellUSDG", async () => {
    await expect(
      vault.connect(user0).sellUSDG(bnb.address, user1.address)
    ...
    await expect(
      vault.connect(user0).sellUSDG(btc.address, user1.address)
    ...
    const tx = await vault
      .connect(user0)
      .sellUSDG(bnb.address, user1.address, { gasPrice: "10000000000" });
