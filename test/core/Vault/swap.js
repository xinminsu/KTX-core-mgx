...
describe("Vault.swap", function () {
  ...
  it("swap", async () => {
    ...
    expect(await vault.usdgAmounts(bnb.address)).eq(
    ...
    await expect(vault.sellUSDG(btc.address, user3.address)).to.be.revertedWith(
  ...
  it("caps max USDG amount", async () => {
    ...
    await expect(
      vault.connect(user0).buyUSDG(bnb.address, user0.address)
