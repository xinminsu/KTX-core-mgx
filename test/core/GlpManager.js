...
describe("GlpManager", function () {
  ...
  it("setGov", async () => {
    await expect(
      glpManager.connect(user0).setGov(user1.address)
  ...
  it("setHandler", async () => {
    await expect(
      glpManager.connect(user0).setHandler(user1.address, true)
    ).to.be.revertedWith("Governable: forbidden");
  ...
  it("setCooldownDuration", async () => {
    await expect(
      glpManager.connect(user0).setCooldownDuration(1000)
    ).to.be.revertedWith("Governable: forbidden");

    await glpManager.setGov(user0.address);

    await expect(
      glpManager.connect(user0).setCooldownDuration(48 * 60 * 60 + 1)
  ...
  it("setAumAdjustment", async () => {
    await expect(
      glpManager.connect(user0).setAumAdjustment(29, 17)
    ).to.be.revertedWith("Governable: forbidden");
  ...
  it("addLiquidity, removeLiquidity", async () => {
    ...
    await dai
      .connect(user0)
      .approve(glpManager.address, expandDecimals(100, 18));
    ...
    expect(await usdg.balanceOf(glpManager.address)).eq(
    ...
    await expect(
      glp.connect(user1).transfer(user2.address, expandDecimals(1, 18))
    ...
    await expect(
      glpManager.connect(user2).removeLiquidity(
        btc.address,
