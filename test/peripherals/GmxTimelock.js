...
describe("GmxTimelock", function () {
  ...
  it("setBuffer", async () => {
    ...
    await expect(
      timelock0.connect(user1).setBuffer(7 * 24 * 60 * 60 + 10)
    ).to.be.revertedWith("GmxTimelock: invalid _buffer");
  ...
  it("setIsAmmEnabled", async () => {
    await expect(
      timelock.connect(user0).setIsAmmEnabled(vaultPriceFeed.address, false)
    ).to.be.revertedWith("GmxTimelock: forbidden");

    expect(await vaultPriceFeed.isAmmEnabled()).eq(false);
    await timelock
      .connect(wallet)
      .setIsAmmEnabled(vaultPriceFeed.address, false);
    expect(await vaultPriceFeed.isAmmEnabled()).eq(false);
  });
  ...
  it("setMaxStrictPriceDeviation", async () => {
    await expect(
      timelock
        .connect(user0)
        .setMaxStrictPriceDeviation(vaultPriceFeed.address, 100)
    ).to.be.revertedWith("GmxTimelock: forbidden");
  ...
  it("setPriceSampleSpace", async () => {
    await expect(
      timelock.connect(user0).setPriceSampleSpace(vaultPriceFeed.address, 0)
    ).to.be.revertedWith("GmxTimelock: forbidden");

    expect(await vaultPriceFeed.priceSampleSpace()).eq(3);
    await timelock
      .connect(wallet)
      .setPriceSampleSpace(vaultPriceFeed.address, 1);
  ...
  it("setIsLeverageEnabled", async () => {
    ...
    await expect(
      timelock.connect(user1).setIsLeverageEnabled(vault.address, false)
    ).to.be.revertedWith("GmxTimelock: forbidden");
    ...
    await expect(
      timelock.connect(user1).addExcludedToken(user2.address)
    ).to.be.revertedWith("GmxTimelock: forbidden");
  });
  ...
  it("setFundingRate", async () => {
    await expect(
      timelock
        .connect(user0)
        .setFundingRate(vault.address, 59 * 60, 100000, 100000)
    ).to.be.revertedWith("GmxTimelock: forbidden");
  ...
  it("processMint", async () => {
    ...
    await expect(
      timelock
        .connect(user0)
        .processMint(gmx.address, user1.address, expandDecimals(100, 18))
    ).to.be.revertedWith("GmxTimelock: forbidden");
    ...
    await expect(
      timelock
        .connect(user0)
        .signalMint(gmx.address, user1.address, expandDecimals(100, 18))
    ).to.be.revertedWith("GmxTimelock: forbidden");
    ...
    await expect(
      timelock
        .connect(wallet)
        .processMint(gmx.address, user2.address, expandDecimals(100, 18))
    ).to.be.revertedWith("GmxTimelock: action not signalled");

    await expect(
      timelock
        .connect(wallet)
        .processMint(gmx.address, user1.address, expandDecimals(101, 18))
    ).to.be.revertedWith("GmxTimelock: action not signalled");
    ...
    const action1 = ethers.utils.solidityKeccak256(
      ["string", "address", "address", "uint256"],
      ["mint", gmx.address, user1.address, expandDecimals(100, 18)]
    );
  ...
  it("setGov", async () => {
    ...
    await expect(
      timelock.connect(wallet).signalSetGov(vault.address, user1.address)
    ).to.be.revertedWith("GmxTimelock: forbidden");

    await timelock
      .connect(tokenManager)
      .signalSetGov(vault.address, user1.address);
    ...
    await timelock
      .connect(tokenManager)
      .signalSetGov(vault.address, user2.address);
  ...
  it("withdrawToken", async () => {
    ...
    await expect(
      timelock
        .connect(user0)
        .withdrawToken(gmx.address, bnb.address, user0.address, 100)
    ).to.be.revertedWith("GmxTimelock: forbidden");
    ...
    await expect(
      timelock
        .connect(user0)
        .signalWithdrawToken(gmx.address, bnb.address, user0.address, 100)
    ).to.be.revertedWith("GmxTimelock: forbidden");
    ...
    await expect(
      timelock
        .connect(wallet)
        .withdrawToken(gmx.address, dai.address, user0.address, 100)
    ).to.be.revertedWith("GmxTimelock: action not signalled");

    await expect(
      timelock
        .connect(wallet)
        .withdrawToken(gmx.address, bnb.address, user1.address, 100)
    ).to.be.revertedWith("GmxTimelock: action not signalled");

    await expect(
      timelock
        .connect(wallet)
        .withdrawToken(gmx.address, bnb.address, user0.address, 101)
    ).to.be.revertedWith("GmxTimelock: action not signalled");
  ...
  it("addPlugin", async () => {
    await expect(
      timelock.connect(user0).addPlugin(router.address, user1.address)
    ).to.be.revertedWith("GmxTimelock: forbidden");

    await expect(
      timelock.connect(wallet).addPlugin(router.address, user1.address)
    ).to.be.revertedWith("GmxTimelock: action not signalled");

    await expect(
      timelock.connect(user0).signalAddPlugin(router.address, user1.address)
    ).to.be.revertedWith("GmxTimelock: forbidden");

    await timelock
      .connect(wallet)
      .signalAddPlugin(router.address, user1.address);
    ...
    await expect(
      timelock.connect(wallet).addPlugin(user2.address, user1.address)
    ).to.be.revertedWith("GmxTimelock: action not signalled");

    await expect(
      timelock.connect(wallet).addPlugin(router.address, user2.address)
    ).to.be.revertedWith("GmxTimelock: action not signalled");
    ...
    await timelock
      .connect(wallet)
      .signalAddPlugin(router.address, user2.address);
  ...
  it("addExcludedToken", async () => {
    const gmx = await deployContract("GMX", []);
    await expect(
      timelock.connect(user0).addExcludedToken(gmx.address)
    ).to.be.revertedWith("GmxTimelock: forbidden");
  ...
  it("setInPrivateTransferMode", async () => {
    ...
    await expect(
      timelock.connect(user0).setInPrivateTransferMode(gmx.address, true)
    ).to.be.revertedWith("GmxTimelock: forbidden");

    await expect(
      timelock.connect(wallet).setInPrivateTransferMode(gmx.address, true)
    ...
    await expect(
      gmx.connect(user0).transfer(user1.address, 100)
