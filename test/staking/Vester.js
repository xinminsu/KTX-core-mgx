...
describe("Vester", function () {
  ...
  it("deposit, claim, withdraw", async () => {
    ...
    expect(await vester.balanceOf(user0.address)).eq(
      expandDecimals(1000, 18).sub(gmxAmount)
    );
  ...
  it("handles pairing", async () => {
    ...
    await gmx
      .connect(user1)
      .approve(stakedGmxTracker.address, expandDecimals(500, 18));
    ...
    expect(await stakedGmxTracker.claimable(user0.address)).gt(
      expandDecimals(1190, 18)
    );
    expect(await stakedGmxTracker.claimable(user0.address)).lt(
    ...
    expect(await feeGmxTracker.balanceOf(user0.address)).eq(
      expandDecimals(1500, 18)
    );
    ...
    expect(await feeGmxTracker.balanceOf(user0.address)).gt(
      expandDecimals(499, 18)
    );
    expect(await feeGmxTracker.balanceOf(user0.address)).lt(
      expandDecimals(501, 18)
    );
    expect(await feeGmxTracker.balanceOf(vester.address)).gt(
      expandDecimals(999, 18)
    );
    expect(await feeGmxTracker.balanceOf(vester.address)).lt(
  ...
  it("handles existing pair tokens", async () => {
    ...
    expect(feeGmxTrackerBalance).eq(
      await feeGmxTracker.balanceOf(user0.address)
    );

    await expect(
      rewardRouter.connect(user0).unstakeGmx(expandDecimals(2, 18))
