...
describe("BonusDistributor", function () {
  ...
  it("distributes bonus", async () => {
    ...
    await gmx
      .connect(user0)
      .approve(stakedGmxTracker.address, expandDecimals(1001, 18));
    await expect(
      stakedGmxTracker
    ...
    expect(await bonusGmxTracker.claimable(user0.address)).gt(
      "2730000000000000000"
    ); // 2.73, 1000 / 365 => ~2.74
    expect(await bonusGmxTracker.claimable(user0.address)).lt(
      "2750000000000000000"
    ); // 2.75
