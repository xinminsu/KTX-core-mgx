...
describe("ReferralStorage", function () {
  ...
  it("Sets new handler", async () => {
    await expect(
      referralStorage.connect(user0).setHandler(user1.address, true)
    ).to.be.revertedWith("Governable: forbidden");

    expect(await referralStorage.isHandler(user0.address)).to.be.false;
    await expect(
      referralStorage.connect(user1).setHandler(user0.address, true)
    ).to.be.revertedWith("Governable: forbidden");
  ...
  it("setTier", async () => {
    await expect(
      referralStorage.connect(user0).setTier(0, 1000, 5000)
    ).to.be.revertedWith("Governable: forbidden");
  ...
  it("setReferrerTier", async () => {
    await expect(
      referralStorage.connect(user0).setReferrerTier(user0.address, 2)
    ).to.be.revertedWith("Governable: forbidden");
  ...
  it("setReferrerDiscountShare", async () => {
    await expect(
      referralStorage.connect(user0).setReferrerDiscountShare(10001)
  ...
  it("setTraderReferralCode", async () => {
    ...
    expect(await referralStorage.traderReferralCodes(user1.address)).eq(
      HashZero
    );
    await referralStorage
      .connect(user0)
      .setTraderReferralCode(user1.address, code);
  ...
  it("Registers code", async () => {
    await expect(
      referralStorage.connect(user0).registerCode(HashZero)
    ...
    await expect(
      referralStorage.connect(user0).registerCode(code)
  ...
  it("setCodeOwner", async () => {
    ...
    await expect(
      referralStorage.connect(user1).setCodeOwner(HashZero, user2.address)
    ).to.be.revertedWith("ReferralStorage: invalid _code");

    await expect(
      referralStorage.connect(user1).setCodeOwner(code, user2.address)
    ).to.be.revertedWith("ReferralStorage: forbidden");
  ...
  it("govSetCodeOwner", async () => {
    ...
    await expect(
      referralStorage.connect(user1).registerCode(HashZero)
    ).to.be.revertedWith("ReferralStorage: invalid _code");

    await expect(
      referralStorage.connect(user1).govSetCodeOwner(code, user2.address)
    ).to.be.revertedWith("Governable: forbidden");
  ...
  it("timelock.setTier", async () => {
    ...
    await expect(referralStorage.setTier(1, 12, 20)).to.be.revertedWith(
      "Governable: forbidden"
    );

    await expect(
      timelock.connect(user0).setTier(referralStorage.address, 1, 12, 20)
  ...
  it("timelock.setReferrerTier", async () => {
    ...
    await expect(
      referralStorage.setReferrerTier(user1.address, 2)
    ).to.be.revertedWith("Governable: forbidden");

    await expect(
      timelock
        .connect(user0)
        .setReferrerTier(referralStorage.address, user1.address, 2)
    ).to.be.revertedWith("Timelock: forbidden");
  ...
  it("timelock.govSetCodeOwner", async () => {
    ...
    await expect(
      referralStorage.govSetCodeOwner(code, user1.address)
    ).to.be.revertedWith("Governable: forbidden");

    await expect(
      timelock
        .connect(user0)
        .govSetCodeOwner(referralStorage.address, code, user1.address)
    ).to.be.revertedWith("Timelock: forbidden");
